...
const InputBox: React.FC<InputBoxProps> = ({ name, inputs, outputs }) => {
  ...
  const { handleTransaction } = useTransaction(contractStore.address, contractStore.abi)
  ...
  }, [setError, handleTransaction, setResultData])
...
const ReadTable: React.FC = observer(() => {
  const { contractStore } = useStore()
  const [functions, setFunction] = useState<any[]>([]);

  useEffect(() => {

    const readList = contractStore.abiParser?.readFunctions.map(
      ({ name, inputs , outputs}) => {
        return {
          name,
          inputs,
